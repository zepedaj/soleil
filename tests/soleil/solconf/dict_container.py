...
class TestRawKeyPatterns(TestCase):
    def test_split_raw_key(self):
        for raw_key, expected in [
            ("_", {"key": "_", "types": None, "modifiers": None}),
            ("_::", {"key": "_", "types": None, "modifiers": None}),
            ("my_key", {"key": "my_key", "types": None, "modifiers": None}),
            ("my_key:int", {"key": "my_key", "types": "int", "modifiers": None}),
            (
                'my_key:"my.xerializer:Type"',
                {"key": "my_key", "types": '"my.xerializer:Type"', "modifiers": None},
            ),
            (
            ...
            self.assertEqual(mdl.KeyNode._split_raw_key(raw_key), expected)
...
class TestKeyNode(TestCase):
    ...
    def test_modify_exceptions(self):
        ...
        with self.assertRaisesRegex(
            exceptions.RawKeyComponentError,
            re.escape(
        ...
        with self.assertRaisesRegex(
            ...
            + r"\w+"
            + re.escape(">, 4)` to node `ParsedNode@'abc'`."),
        ):
...
class TestDictContainer(TestCase):
    ...
    def test_keynode_change_key(self):
        ...
        # Test removal
        self.assertEqual({"my_key"}, set(container.children))
        container.remove(node2)
        self.assertEqual(set(), set(container.children))
...
class TestFunctions(TestCase):
    def test_merge_decorator_values(self):
        parser = Parser()
        self.assertEqual(mdl.merge_decorator_values("int", parser.safe_eval), (int,))
        self.assertEqual(
            mdl.merge_decorator_values("int,float", parser.safe_eval), (int, float)
        )
        self.assertEqual(
            mdl.merge_decorator_values(["int", "float"], parser.safe_eval), (int, float)
        )
        self.assertEqual(
            mdl.merge_decorator_values(["int", "float,str"], parser.safe_eval),
            (int, float, str),
        )
        self.assertEqual(mdl.merge_decorator_values("None", parser.safe_eval), None)
