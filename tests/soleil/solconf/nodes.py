...
class TestNode(TestCase):
    def test_patterns(self):
        ...
        # Component
        for x in ["c", "1", "abcd"]:
            self.assertIsNotNone(re.fullmatch(mdl.Node._REF_STR_COMPONENT_PATTERN, x))
            self.assertIsNotNone(
                re.fullmatch(mdl.Node._REF_STR_COMPONENT_PATTERN_OR_DOTS, x)
            )
            self.assertIsNotNone(re.fullmatch(mdl.Node._FULL_REF_STR_PATTERN, x))
    ...
    def test_call(self):
        ...
        node = KeyNode(
            raw_key="my_name",
            value=(value_node := mdl.ParsedNode("$:n_", parser=parser)),
            parser=parser,
        )
    ...
    def test_qual_name(self):
        ...
        for root, ref_str__exp_node__exp_type__exp_value in [
            # Dict-of-dict
            (
                _root := SolConf(_raw_data := {"node0": {"node1": 1}}).node_tree,
                [
                    ("", _root, DictContainer, _raw_data),
                    ("node0", _root["*node0"].value, DictContainer, _raw_data["node0"]),
                    ("*node0", _root["*node0"], KeyNode, ("node0", _raw_data["node0"])),
                    (
                        "node0.node1",
                        _root["*node0"].value["*node1"].value,
                        ParsedNode,
                        _raw_data["node0"]["node1"],
                    ),
                ],
            ),
            # ParsedNode
            ...
            for (
                ...
                expected_type,
                expected_value,
            ) in ref_str__exp_node__exp_type__exp_value:
                #
