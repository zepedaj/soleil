...
def build_config_files(
    root_updates=None,
    root_expected_updates=None,
    file1_updates=None,
    file1_expected_updates=None,
    file2_updates=None,
    file2_expected_updates=None,
):
...
class TestModifiers(TestCase):
    def test_parent(self):
        ...
        self.assertEqual(
            resolved := key_node.resolve(), ("my_name", (value_node, key_node))
        )
    ...
    def test_promote(self):
        ...
        sc = SolConf({"a": {"x:bool:promote": False}, "b": '$: r_["a"]()'})
        self.assertEqual(sc(), {"a": False, "b": False})

        sc = SolConf({"a": {"x:bool:promote,hidden": False}, "b": '$: r_["a"]()'})
    ...
    def test_hidden_and_promote_order(self):
        ...
        for modifiers_str in ["promote,hidden", "hidden,promote"]:
            sc = SolConf(
                {"a0": {f"x:bool:{modifiers_str}": False}, "a1": "$: r_['a0']()"}
            )
    ...
    def test_fuse(self):
        ...
        # Fuse
        sc = SolConf({"_::fuse": {"value": 1, "types": "int", "modifiers": "noop"}})
        ...
        # Fuse, fused promote
        sc = SolConf({"_::fuse": {"value": 1, "types": "int", "modifiers": "promote"}})
        ...
        sc = SolConf(
            {"_::fuse,promote": {"value": 1, "types": "int", "modifiers": "noop"}}
        )
        ...
        # Fuse, partial keys
        sc = SolConf({"_::fuse,promote": {"value": 1}})
    ...
    def test_docs(self):
        ...
        # Fuse-based syntax
        sc_fused = SolConf(
            {"base::fuse": {"value": "$: 1+2", "types": "int", "modifiers": "noop"}}
        )
        ...
        self.assertEqual(
            (sc_fused["base"].types, sc_rk["base"].types), ((int,), (int,))
        )

        self.assertEqual(
            (sc_fused["base"].modifiers, sc_rk["base"].modifiers), ((noop,), (noop,))
        )

        self.assertEqual(
            (sc_fused["base"].raw_value, sc_rk["base"].raw_value), ("$: 1+2", "$: 1+2")
        )

        self.assertEqual((sc_fused(), sc_rk()), ({"base": 3}, {"base": 3}))
        ...
        self.assertEqual(sc_fused["base"].types, (int, float))
    ...
    def test_derives(self):
        ...
        with file_structure(
            ...
            self.assertEqual(
                out, {"x": {"c": 2, "d": 3, "e": 4}, "y": {"c": 2, "d": 3, "e": 4}}
            )
        ...
        with file_structure(
            ...
            out = SolConf.load(path_mappings["config_source.yaml"])()
            self.assertEqual(out, {"x": {"c": 2, "d": 2}, "y": {"c": -2, "d": 2}})
